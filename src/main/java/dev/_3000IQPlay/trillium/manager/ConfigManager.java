...
public class ConfigManager implements Util {
    ...
    public String config = "Trillium/config/";
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry<String, JsonElement> entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    Trillium.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName),""));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                continue;
    ...
    public void loadConfig(String name,boolean firstrun) {
        ...
        Trillium.moduleManager.onUnload();
        Trillium.moduleManager.onUnloadPost();

        currentcfg = name;
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("Trillium").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        if (files.contains(new File("Trillium/" + name + "/"))) {
            this.config = "Trillium/" + name + "/";
        } else {
        ...
        Trillium.moduleManager.onLoad();
        saveCurrentConfig();
    }

    public boolean configExists(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("Trillium").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        return files.contains(new File("Trillium/" + name + "/"));
    }
    ...
    public void deleteConfig(String name){
        ...
        path.renameTo(new File("Trillium/oldcfg" + Math.random()*1000));
    }
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("Trillium/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("Trillium", ""));
                writer.close();
            } else {
