package dev._3000IQPlay.trillium.gui.particles;
...
public class Particle {
    ...
    public void tick(int delta, float speed) {
        ...
        if (pos.getX() + Particles.getInstance().scale1.getValue() > sr.getScaledWidth()) velocity = new Vector2f(-velocity.x, velocity.y);
        if (pos.getX() - Particles.getInstance().scale1.getValue() < 0) velocity = new Vector2f(-velocity.x, velocity.y);

        if (pos.getY() + Particles.getInstance().scale1.getValue() > sr.getScaledWidth()) velocity = new Vector2f(velocity.x, -velocity.y);
        if (pos.getY() - Particles.getInstance().scale1.getValue() < 0) velocity = new Vector2f(velocity.x, -velocity.y);
    }
