...
import static dev._3000IQPlay.trillium.Trillium.alts;
import static dev._3000IQPlay.trillium.gui.clickui.elements.SliderElement.removeLastChar;
...
public class GuiAltManager extends GuiScreen
{
    ...
    public GuiAltManager() {
        try {
            if(Trillium.moduleManager != null){
                switch (Trillium.moduleManager.getModuleByClass(MainSettings.class).shaderMode.getValue()) {
                    case Smoke:
                        backgroundShader = new MainMenuShader("mainmenu/mainmenu.fsh");
                        break;
                }
            }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        this.backgroundShader.useShader(sr.getScaledWidth() * 2, sr.getScaledHeight() * 2, (float)mouseX, (float)mouseY, (float)(System.currentTimeMillis() - Trillium.initTime) / 1000.0F);
        checkMouseWheel();
    ...
    public void mouseClicked(int x,int y, int button){
        ...
        if (x >= sr.getScaledWidth() / 2 + 4 && x <= sr.getScaledWidth() / 2 + 111 && y >= sr.getScaledHeight() - 135 && y <= sr.getScaledHeight() - 100) {
            String name = "Trillium" + (int)(Math.random() * 10000);
            alts.add(name);
            try {
                new Thread(() -> TrilliumUtils.saveUserAvatar("https://minotar.net/helm/" + name +"/16.png", name));
            } catch (Exception e){}
        }
        if (x >= sr.getScaledWidth() / 2 - 120 && x <= sr.getScaledWidth() / 2 + 102 && y >= sr.getScaledHeight() - 96 && y <= sr.getScaledHeight() - 61) {
            this.mc.displayGuiScreen(new TrilliumMenu());
        }
    }
    ...
    public void keyTyped(char chr, int keyCode) {
        if (listening) {
            switch (keyCode) {
                ...
                case 28: {
                    alts.add(add_name);
                    TrilliumUtils.saveUserAvatar("https://minotar.net/helm/" + add_name + "/16.png", add_name);
                    add_name = "";
                    listening = false;
                }
                case 14: {
