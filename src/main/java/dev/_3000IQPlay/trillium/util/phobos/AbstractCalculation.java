...
import static dev._3000IQPlay.trillium.modules.combat.AutoCrystal.timercheckerfg;
import static dev._3000IQPlay.trillium.modules.combat.AutoCrystal.timercheckerwfg;
import static dev._3000IQPlay.trillium.util.Util.mc;
import static dev._3000IQPlay.trillium.util.phobos.CalculationMotion.rayTraceTo;
import static dev._3000IQPlay.trillium.util.phobos.RotationUtil.getRotationsToTopMiddle;
import static dev._3000IQPlay.trillium.util.phobos.RotationUtil.rayTraceWithYP;
...
public abstract class AbstractCalculation<T extends CrystalData> extends Finishable
{
    ...
    protected boolean breakCheck()
    {
        return module.attack.getValue()
                && !noBreak
                && Trillium.switchManager.getLastSwitch() >= module.cooldown.getValue()
    ...
    protected boolean preSpecialCheck()
    {
        return !placing
                ...
                || module.targetMode.getValue() == AutoCrystal.Target.Damage)
                && !fallback;
    }
    ...
    protected boolean place(PlaceData data) //TODO вызывается
    {
        ...
        if (antiTotem != null)
        {
            ...
            Command.sendMessage("Attempting AntiTotem: "
                    + (player == null ? "null" : player.getName()));

            module.antiTotemHelper.setTargetPos(antiTotem.getPos());
            place(antiTotem, player, !module.noRotateNigga(AutoCrystal.ACRotate.Place), rotating || scheduling, false);
                ...
                && module.forceTimer.passedMs(module.attempts.getValue()))
        {
                    ...
                    data.getForceData().entrySet())
            {
                ...
                place(first, entry.getKey(), !module.noRotateNigga(AutoCrystal.ACRotate.Place), rotating || scheduling, module.forceSlow.getValue());
    ...
    protected boolean placeNoAntiTotem(PlaceData data, MineSlots liquid) //TODO вызывается
    {
        ...
        for (PositionData d : data.getData())
        {
            if (module.override.getValue()
                    ...
                    && (!module.override.getValue() || d.getMaxDamage() < d.getHealth())) {
                continue;
            }
        ...
        if (firstData != null && !module.damageSyncHelper.isSyncing(firstData.getMaxDamage(), module.damageSync.getValue()) && (liquid == null || module.minDamage.getValue() <= firstData.getMaxDamage()))
        {
            ...
            place(firstData, firstData.getTarget(), !module.noRotateNigga(AutoCrystal.ACRotate.Place), rotating || scheduling, slow, slow ? firstData.getMaxDamage() : Float.MAX_VALUE, liquidBreak);
            return false;
        }

        Optional<PositionData> shield;
        if (module.shield.getValue()
                && module.shieldTimer.passedMs(module.shieldDelay.getValue())
    ...
    protected void place(PositionData data, EntityPlayer target, boolean rotate, boolean schedule, boolean resetSlow, float damage, MutableWrapper<Boolean> liquidBreak) {
        place(data, target, rotate, schedule, resetSlow, damage, liquidBreak, false);
    }
    ...
    protected void place(PositionData data, EntityPlayer target, boolean rotate, boolean schedule, boolean resetSlow, float damage, MutableWrapper<Boolean> liquidBreak, boolean shield) {
       ...
       // timercheckerfg.reset();
        ...
        module.damageSyncHelper.setSync(pos, data.getMaxDamage(), module.newVerEntities.getValue());
    ...
    {
        double x = Trillium.positionManager.getX();
        double y = Trillium.positionManager.getY() + (module.placeRangeEyes.getValue() ? RotationUtil.getRotationPlayer().getEyeHeight() : 0);
        double z = Trillium.positionManager.getZ();
