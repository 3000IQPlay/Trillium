...
public abstract class AbstractBreakHelper<T extends CrystalData> implements IBreakHelper<T>
{
                                ...
                                List<EntityPlayer> friends)
    {
        ...
        {
            if (!(crystal instanceof EntityEnderCrystal) || EntityUtil.isDead(crystal) && (!module.countDeadCrystals.getValue() || module.countDeathTime.getValue() && (EntityUtil.isDead(crystal) && Trillium.setDeadManager.passedDeathTime(crystal, module.getDeathTime()) || ((IEntity) crystal).isPseudoDead() && ((IEntity) crystal).getPseudoTime().passedMs(module.getDeathTime()))))
            {
                continue;
            }
