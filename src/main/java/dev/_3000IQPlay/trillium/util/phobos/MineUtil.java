...
import dev._3000IQPlay.trillium.mixin.mixins.IBlock;
import dev._3000IQPlay.trillium.mixin.mixins.IItemTool;
...
public class MineUtil
{
    ...
    {
        Set<Block> blocks = getField(ItemPickaxe.class, null, 0);
        for (Block block : blocks)
        {
            ((IBlock) block).setHarvestLevelNonForge("pickaxe", 0);
        }

        blocks = getField(ItemSpade.class, null, 0);
        for (Block block : blocks)
        {
            ((IBlock) block).setHarvestLevelNonForge("shovel", 0);
        }

        blocks = getField(ItemAxe.class, null, 0);
        for (Block block : blocks)
        {
