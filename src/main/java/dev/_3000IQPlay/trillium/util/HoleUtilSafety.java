...
public class HoleUtilSafety
	...
	public static boolean isAir(BlockPos blockPos) {
        return HoleUtilSafety.mc.world.getBlockState(blockPos).getBlock() == Blocks.AIR;
    }
    ...
    public static boolean is2x2(BlockPos blockPos, boolean bl) {
        if (bl && !HoleUtilSafety.isAir(blockPos)) {
            return false;
        }
        ...
        boolean bl2 = HoleUtilSafety.isAir(blockPos2);
        if (bl2 && HoleUtilSafety.is2x2Partial(blockPos2)) {
            return true;
        }
        BlockPos blockPos3 = blockPos.add(0, 0, -1);
        boolean bl3 = HoleUtilSafety.isAir(blockPos3);
    ...
    public static boolean is2x1(BlockPos blockPos, boolean bl) {
        if (bl) {
            if (!HoleUtilSafety.isAir(blockPos)) {
                return false;
            }
            if (!HoleUtilSafety.isAir(blockPos.up())) {
                return false;
            }
            if (HoleUtilSafety.isAir(blockPos.down())) {
                return false;
            }
        }
        int n = 0;
        for (EnumFacing enumFacing : EnumFacing.HORIZONTALS) {
            ...
            if (HoleUtilSafety.isAir(blockPos2)) {
                if (!HoleUtilSafety.isAir(blockPos2.up())) {
                    return false;
                }
                if (HoleUtilSafety.isAir(blockPos2.down())) {
                    return false;
                }
    ...
    public static boolean is2x2Partial(BlockPos blockPos) {
        ...
        for (BlockPos blockPos2 : hashSet) {
            if (HoleUtilSafety.isAir(blockPos2) && HoleUtilSafety.isAir(blockPos2.up()) && !HoleUtilSafety.isAir(blockPos2.down())) {
                if (HoleUtilSafety.isAir(blockPos2.up(2))) {
                    bl = true;
                }
                for (EnumFacing enumFacing : EnumFacing.HORIZONTALS) {
    ...
    public static boolean[] isHole(BlockPos blockPos, boolean bl) {
        ...
        if (!HoleUtilSafety.isAir(blockPos) || !HoleUtilSafety.isAir(blockPos.up()) || bl && !HoleUtilSafety.isAir(blockPos.up(2))) {
            return arrbl;
        }
