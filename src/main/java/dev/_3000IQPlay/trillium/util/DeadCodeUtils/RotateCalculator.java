...
public class RotateCalculator {
    ...
    public static void a(Entity entity, boolean bl, boolean bl2, float f2) {
        ...
        if (f3 < 0.1f) {
            fArray = a(a(entity.getEntityBoundingBox()), true);
            f4 = fArray[0];
            f5 = fArray[1];
        }
        if (bl) {
        ...
        fArray = a(new float[]{entityPlayerSP.rotationYaw, entityPlayerSP.rotationPitch}, new float[]{f4, f5}, (float)(10 + RandomUtils.nextInt(1,6)));
        if (fArray == null) {
            return;
        }
    ...
    public static boolean a(Entity entity, Entity entity2, double d2) {
        double d3 = a(entity.rotationYaw, a(entity, entity2.posX, entity2.posY, entity2.posZ)[0]);
        return d3 > 0.0 && d3 < (d2 *= 0.5) || -d2 < d3 && d3 < 0.0;
    }

    public static boolean a(Entity entity, double d2) {
        return a((Entity)mc.player, entity, d2);
    }

    public static boolean a(BlockPos blockPos, double d2) {
        double d3 = a(mc.player.rotationYaw, a((Entity)mc.player, blockPos.getX(), blockPos.getY(), (double)blockPos.getZ())[0]);
        return d3 > 0.0 && d3 < (d2 *= 0.5) || -d2 < d3 && d3 < 0.0;
    }
    ...
    public static float as(float f2, float f3, float f4) {
        float f5;
    ...
    public static float[] a(float[] fArray, float[] fArray2, float f2) {
        double d2 = b(fArray2[0], fArray[0]);
        double d3 = b(fArray2[1], fArray[1]);
        boolean bl = false;
    ...
    public static double d(float f2, float f3) {
        return Math.sqrt(Math.pow(Math.abs(b(ef.d() % 360.0f, f2)), 2.0) + Math.pow(Math.abs(b(ef.e(), f3)), 2.0));
    }
    ...
    public static float[] a(Entity entity, cy_0 cy_02, float f2) {
        ...
        if (entity instanceof EntityLivingBase) {
            ...
            float f3 = RandomUtils.nextFloat((float)(entityLivingBase.posY + (double)(entityLivingBase.getEyeHeight() / 1.5f)), (float)(entityLivingBase.posY + (double)entityLivingBase.getEyeHeight() - (double)(entityLivingBase.getEyeHeight() / 3.0f)));
            d2 = (double)f3 - (entityPlayerSP.posY + (double)entityPlayerSP.getEyeHeight());
        } else {
            d2 = (double)RandomUtils.nextFloat((float)entity.getEntityBoundingBox().minY, (float)entity.getEntityBoundingBox().maxY) - (entityPlayerSP.posY + (double)entityPlayerSP.getEyeHeight());
        }
        switch (cy_02) {
            case a: {
                d2 += 0.6;
        ...
        if ((double)f2 > 0.0) {
            f4 += RandomUtils.nextFloat(-f2, f2);
            f5 += RandomUtils.nextFloat(-f2, f2);
        }
