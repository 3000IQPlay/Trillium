...
import dev._3000IQPlay.trillium.mixin.mixins.IEntityLivingBase;
...
public class PhysicsUtil {
    public static void runPhysicsTick() {
        int lastSwing = ((IEntityLivingBase) mc.player).getTicksSinceLastSwing();
        int useCount  = ((IEntityLivingBase) mc.player).getActiveItemStackUseCount();
        ...
        ((IEntityLivingBase) mc.player).setTicksSinceLastSwing(lastSwing);
        ((IEntityLivingBase) mc.player).setActiveItemStackUseCount(useCount);
        ...
        ((IEntityPlayerSP) mc.player).invokeOnUpdateWalkingPlayer();
    }
}