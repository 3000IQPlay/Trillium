...
public class HoleSnap
    ...
    public void onTick() {
        ...
        if (this.mode.getValue() == Mode.Instant) {
            blockPos2 = Trillium.holeManager.calcHoles().stream().min(Comparator.comparing(blockPos -> HoleSnap.mc.player.getDistance((double)blockPos.getX(), (double)blockPos.getY(), (double)blockPos.getZ()))).orElse(null);
            if (blockPos2 != null) {
    ...
    public void onEnable() {
        ...
        if (this.SpeedCheck.getValue().booleanValue() && Speed.getInstance().isOn()) {
            Speed.getInstance().disable();
