...
public class TargetStrafe extends Module {
    ...
    public Setting<Boolean> autoJump = this.register(new Setting<>("Auto Jump", true));
    ...
    public Setting<Float> trgrange = register(new Setting("TargetRange", 3.8F, 0.1F, 7.0F));
    public Setting<Boolean> drawradius = this.register(new Setting<>("Draw Radius", true));
    ...
    public Setting<Integer> velocitydecrement = register(new Setting("BoostDecrement", 5, 0, 5000));
    ...
    public TargetStrafe() {
        super("TargetStrafe", "Strafes around your target", Module.Category.MOVEMENT, true, false, false);
    }
    ...
    public void onUpdateWalkingPlayerPre(EventPreMotion event) {
        ...
        if (autoThirdPerson.getValue()) {
            if (strafeTarget.getHealth() > 0.0f && mc.player.getDistance(strafeTarget) <= trgrange.getValue() && mc.player.getHealth() > 0.0f) {
                if (Trillium.moduleManager.getModuleByClass(Aura.class).isEnabled()) {
        ...
        if (mc.player.getDistance(strafeTarget) <= trgrange.getValue()) {
            if (EntityUtil.getHealth(strafeTarget) > 0) {
                if (autoJump.getValue() && (Trillium.moduleManager.getModuleByClass(Aura.class).isEnabled())) {
