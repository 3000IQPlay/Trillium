...
public class AutoXP extends Module {
    public AutoXP() {
        super("AutoXP", "Auto XPs armor for lazy monkeys", Module.Category.PLAYER, true, false, false);
    }
    ...
    private final Setting<Integer> threshold = this.register(new Setting<>("Percentage", 100, 0, 100));
    ...
    public void onRender2D(Render2DEvent e) {
        if (PlayerUtils.isKeyDown(subBind.getValue().getKey())) {
            ...
            AutoXP.mc.getRenderItem().renderItemIntoGUI(new ItemStack(Items.EXPERIENCE_BOTTLE), waterMarkZ2.getValue() + 70 + 11, waterMarkZ1.getValue() + 17);
    ...
    private int getExpCount() {
        int expCount = 0;
        for (int i = 0; i < 45; ++i) {
            if (AutoXP.mc.player.inventory.getStackInSlot(i).getItem().equals(Items.EXPERIENCE_BOTTLE)) {
                expCount = expCount + AutoXP.mc.player.inventory.getStackInSlot(i).getCount();
            }
        }
        if (AutoXP.mc.player.getHeldItemOffhand().getItem().equals(Items.EXPERIENCE_BOTTLE)) {
            ++expCount;
        }
        return expCount;
    }
