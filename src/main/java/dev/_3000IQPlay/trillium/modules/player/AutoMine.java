...
public class AutoMine extends Module {
    ...
    public void onPreMotion(EventPreMotion event){
        ...
        if( !switchbool.getValue() || checkPickaxe( ) )
        {
            ...
            {
                return player != mc.player && !Trillium.friendManager.isFriend( player.getName( ) ) && Float.compare( mc.player.getDistance( player ), 7.0f ) < 0;
            ...
            if( blockpos2 != null )
            {
                ...
                if( !requirepickaxe.getValue() || mc.player.getHeldItemMainhand( ).getItem( ).equals( Items.DIAMOND_PICKAXE ) )
                {
                    if( blockpos != null )
                    {
                        ...
                        {
                            if( Trillium.moduleManager.getModuleByClass(Speedmine.class).isEnabled() )
                                return;
                        }
                    }
