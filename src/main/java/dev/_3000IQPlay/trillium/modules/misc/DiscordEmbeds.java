...
public class DiscordEmbeds
    ...
    public DiscordEmbeds() {
        super("DiscordEmbeds", "Displays an embed when u hiver over the link", Module.Category.MISC, true, false, false);
    }

    public Setting<Integer> multip = this.register ( new Setting <> ( "Scale", 450, 50, 1280 ) );
    public Setting<Integer> posY = this.register ( new Setting <> ( "PosY", 0, 0, 1000 ) );
    public Setting<Integer> posX = this.register ( new Setting <> ( "PosX", 400, -1000, 1000 ) );

    public  Setting<Boolean> fgbypass = register(new Setting<>("Testing", false));
        ...
        if( message.contains("discordapp") && message.contains(".png")){
            ...
            try {
                ...
                if(Objects.equals(solvename(message), "err")){
                    ITextComponent cancel = new TextComponentString("Image received via THCRYPT [SHOW]");
                    ...
                    ITextComponent cancel = new TextComponentString("<" + solvename(message) + "> sent a picture [Show Discord Image]");
                    cancel.setStyle(cancel.getStyle().setColor(TextFormatting.AQUA).setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, cancel2)));
                    Command.sendIText(cancel);
                }
        ...
        try {
            ...
            if (check) {
                ...
                try {
                    ...
                    ImageIO.write(img, format, new File("Trillium/temp/embeds/" + niggermod + "." + format));
    ...
    public void onRender2D(Render2DEvent e) {
        ...
        if (nado && guiscreen instanceof GuiChat && !Objects.equals(this.filename, "") && !Objects.equals(this.formatfila, "")) {
            ...
            if (this.logo != null) {
                ...
                DiscordEmbeds.drawCompleteImage(xvalue - (nigw/2) + posX.getValue(), yvalue - (nigh/2) + posY.getValue(), (int) nigw, (int) nigh);
            }
        } if(!nado){
            this.logo = null;
        }
