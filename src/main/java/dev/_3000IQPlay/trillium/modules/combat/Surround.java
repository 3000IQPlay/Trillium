...
import dev._3000IQPlay.trillium.manager.MovementManager;
...
public class Surround
	...
	private final Setting<CenterType> centerType = this.register(new Setting<CenterType>("CenterType", CenterType.None));
    ...
    private final dev._3000IQPlay.trillium.util.Timer timer = new dev._3000IQPlay.trillium.util.Timer();
    private final dev._3000IQPlay.trillium.util.Timer retryTimer = new Timer();
    ...
    private boolean placeBlocks(Vec3d pos, Vec3d[] vec3ds, boolean hasHelpingBlocks, boolean isHelping, boolean isExtending) {
        ...
        for (Vec3d vec3d : vec3ds) {
            ...
            switch (BlockUtils.isPositionPlaceable(position, false)) {
                case 1: {
                    ...
                    if (Trillium.speedManager.getSpeedKpH() != 0.0 || isExtending || this.extenders >= 1) continue block5;
                ...
                case 2: {
                    ...
                    gotHelp = this.placeBlocks(pos, BlockUtils.getHelpingBlocks(vec3d), false, true, true);
                }
                case 3: {
                    if (gotHelp) {
                        this.placeBlock(position);
    ...
    private void placeBlock(BlockPos pos) {
        if (this.placements < this.blocksPerTick.getValue()) {
            ...
            this.isSneaking = BlockUtils.placeBlock(pos, this.offHand ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, this.rotate.getValue(), this.noGhost.getValue(), this.isSneaking);
