...
import dev._3000IQPlay.trillium.event.events.EventEntitySpawn;
...
import dev._3000IQPlay.trillium.util.phobos.ThreadUtil;
import dev._3000IQPlay.trillium.util.surround.*;
...
*   Made readable and runable by Pan4ur
**/
	
public class Surround extends Module {
	private final Setting<Page> page = register(new Setting<>("Page", Page.Place));
    public enum Page {Place, CrystalBreak}
	
    public final Setting<Boolean> multiThread = register(new Setting<>("Multi Thread", false, v -> this.page.getValue() == Page.Place));
    private final Setting<Integer> delay = register(new Setting<>("Delay", 0, 0, 100, v -> this.page.getValue() == Page.Place));

    private final Setting<EventMode> eventMode = register(new Setting<>("Event Mode", EventMode.Update, v -> this.page.getValue() == Page.Place));
    public enum EventMode {Tick, Update, SyncEvent}

    private final Setting<Mode> mainMode = register(new Setting<>("Mode", Mode.Normal, v -> this.page.getValue() == Page.Place));
    public enum Mode { High, AntiFacePlace, Dynamic, Cubic, Safe, SemiSafe, Strict, Normal }

    private final Setting<BlockMode> blockMode = register(new Setting<>("Block", BlockMode.Obsidian, v -> this.page.getValue() == Page.Place));
    public enum BlockMode { Obsidian, EnderChest }

    private final Setting<Switch> switchMode = register(new Setting<>("Switch", Switch.Silent, v -> this.page.getValue() == Page.Place));
    public enum Switch { None, Vanilla, Packet, Silent }

    private final Setting<SwitchWhen> switchWhen = register(new Setting<>("SwitchWhen", SwitchWhen.Place, v -> this.page.getValue() == Page.Place));
    public enum SwitchWhen {Place, RunSurround}
	
	public Setting<Boolean> crystalBreaker = register(new Setting<>("CrystalBreaker", true, v -> this.page.getValue() == Page.CrystalBreak));

    private final Setting<CBTimings> cBTimingsMode = register(new Setting<>("CB-Timings", CBTimings.Adaptive, v -> this.page.getValue() == Page.CrystalBreak));
    public enum CBTimings { Sequential, Adaptive }

    private final Setting<CbMode> cbMode = register(new Setting<>("CB-Mode", CbMode.SurroundBlocks, v -> this.page.getValue() == Page.CrystalBreak));
    public enum CbMode { SurroundBlocks, Area }

    private final Setting<CBRotateMode> cBRotateMode = register(new Setting<>("CB-RotateMode", CBRotateMode.Packet));
    public enum CBRotateMode { Client, Packet, Both }

    private final Setting<Rotate> rotateMode = register(new Setting<>("Rotate", Rotate.None, v -> this.page.getValue() == Page.Place));
    public enum Rotate { None, Packet, Silent }

    private final Setting<detectEntityMode> detectEntity = register(new Setting<>("DetectEntity", detectEntityMode.RemoveEntity, v -> this.page.getValue() == Page.Place));
    public enum detectEntityMode {Off, RemoveEntity,SetDead,Both}

    private final Setting<ToggleMode> toggleMode = register(new Setting<>("Toggle", ToggleMode.OffGround, v -> this.page.getValue() == Page.Place));
    public enum ToggleMode { Never, OffGround, OnComplete, Combo, PositiveYChange, YChange}

    public Setting<Boolean> syncronized = register(new Setting<>("Syncronized", false, v -> this.page.getValue() == Page.Place));
    public Setting<Boolean> allEntities = register(new Setting<>("AllEntities", false, v -> this.page.getValue() == Page.Place));
    public Setting<Boolean> extension = register(new Setting<>("Extension", false, v -> this.page.getValue() == Page.Place));
    public Setting<Boolean> safeDynamic = register(new Setting<>("Safe Dynamic", false, v -> this.page.getValue() == Page.Place));
    public Setting<Boolean> smartBlock = register(new Setting<>("Smart Block", false, v -> this.page.getValue() == Page.Place));
    public Setting<Boolean> safeEChest = register(new Setting<>("Safe EChest", false, v -> this.page.getValue() == Page.Place));
    public Setting<Boolean> center = register(new Setting<>("Center", true, v -> this.page.getValue() == Page.Place));
    public Setting<Boolean> smartCenter = register(new Setting<>("SmartCenter", false, v -> this.page.getValue() == Page.Place));
    public Setting<Boolean> smartHelping = register(new Setting<>("SmartHelping", false, v -> this.page.getValue() == Page.Place));
    public Setting<Boolean> fightCA = register(new Setting<>("FightCA", true, v -> this.page.getValue() == Page.Place));
    public Setting<Boolean> detectSound = register(new Setting<>("DetectSound", true, v -> this.page.getValue() == Page.Place));
    public Setting<Boolean> onEntityDestruction = register(new Setting<>("OnEntityDestruction", true, v -> this.page.getValue() == Page.Place));
    public Setting<Boolean> antiCity = register(new Setting<>("AntiCity", false, v -> this.page.getValue() == Page.Place));
    public Setting<Boolean> manipulateWorld = register(new Setting<>("ManipulateWorld", false, v -> this.page.getValue() == Page.Place));
    public Setting<Boolean> postReceive = register(new Setting<>("PostReceive", true, v -> this.page.getValue() == Page.Place));
    public Setting<Boolean> packet = register(new Setting<>("Packet", true, v -> this.page.getValue() == Page.Place));
    public Setting<Boolean> feetBlocks = register(new Setting<>("FeetBlocks", false, v -> this.page.getValue() == Page.Place));
    public Setting<Boolean> down = register(new Setting<>("Down", false, v -> this.page.getValue() == Page.Place));
    public Setting<Boolean> inAir = register(new Setting<>("InAir", false, v -> this.page.getValue() == Page.Place));
    public Setting<Boolean> airMotion = register(new Setting<>("InAirMotionStop", false, v -> this.page.getValue() == Page.Place));
    public Setting<Boolean> cBRotate = register(new Setting<>("CB-Rotate", false, v -> this.page.getValue() == Page.CrystalBreak));
    public Setting<Boolean> cBPacket = register(new Setting<>("CB-Packet", false, v -> this.page.getValue() == Page.CrystalBreak));
    public Setting<Boolean> cientSide = register(new Setting<>("ClientSide", false, v -> this.page.getValue() == Page.Place));
    public final Setting<Boolean> cbTerrain = register(new Setting<>("CB-Terrain", true, v -> this.page.getValue() == Page.CrystalBreak));
    public  final Setting<Boolean> cbNoSuicide = register(new Setting<>("CB-NoSuicide", true, v -> this.page.getValue() == Page.CrystalBreak));
    private final Setting<Float> cBSequentialDelay = register(new Setting<>("CB-SequentialDelay", 1f, 0f, 10f, v -> this.page.getValue() == Page.CrystalBreak));
    private final Setting<Float> cBRange = register(new Setting<>("CB-Range", 3f, 0f, 10f, v -> this.page.getValue() == Page.CrystalBreak));
    private final Setting<Integer> cBDelay = register(new Setting<>("CB-Delay", 0, 0, 500, v -> this.page.getValue() == Page.CrystalBreak));
	public Setting<Boolean> rangeCheck = register(new Setting<>("RangeCheck", false, v -> this.page.getValue() == Page.Place));
    private final Setting<Float> placeRange = register(new Setting<>("PlaceRange", 5f, 2f, 10f, v -> this.rangeCheck.getValue() && this.page.getValue() == Page.Place));
	private final Setting<Float> heightLimit = register(new Setting<>("HeightLimit", 256.0f, 0f, 256.0f, v -> this.page.getValue() == Page.Place));
    private final Setting<Float> toggleHeight = register(new Setting<>("ToggleHeight", 0.4f, 0.1f, 1f, v -> this.page.getValue() == Page.Place));

    private BlockPos startPos;
    private SurroundUtil surroundUtil = new SurroundUtil();
    ...
    public void onSpawnCrystal(EventEntitySpawn event) {
        ...
        List<BlockPos> list = surroundUtil.getBlockPositions(mainMode.getValue());
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (!doesCrystalWantToFuckUs(vec3d, surroundUtil.getBlockPositions(mainMode.getValue()))) {
            return;
        }
        if (syncronized.getValue()) {
            doSynchronized();
    ...
    private void handleSurround() {
        ...
        if(!inAir.getValue() || mc.player.onGround) {
            ...
            List<BlockPos> positions = surroundUtil.getBlockPositions(mainMode.getValue());
            doPlace(positions);
    ...
    public List<BlockPos> getAntiFacePlacePositions() {
        ArrayList<BlockPos> arrayList = new ArrayList<>(16);
        arrayList.addAll(surroundUtil.getBlockPositions(Mode.Normal));
        BlockPos blockPos = new BlockPos(mc.player.posX, mc.player.posY, mc.player.posZ);
