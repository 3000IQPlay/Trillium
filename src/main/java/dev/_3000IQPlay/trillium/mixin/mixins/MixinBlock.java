...
public abstract class MixinBlock
{
    ...
    public void addCollisionBoxToListHook(final IBlockState state,  final World worldIn,  final BlockPos pos,  final AxisAlignedBB entityBox,  final List<AxisAlignedBB> collidingBoxes,  @Nullable final Entity entityIn,  final boolean isActualState,  final CallbackInfo info) {
        if (entityIn != null && Util.mc.player != null && (entityIn.equals((Object)Util.mc.player) || (Util.mc.player.getRidingEntity() != null && entityIn.equals((Object)Util.mc.player.getRidingEntity()))) && Trillium.moduleManager.getModuleByClass(Phase.class).isOn()) {
            info.cancel();
        }
    }
    ...
    public void isFullCubeHook(final IBlockState blockState,  final CallbackInfoReturnable<Boolean> info) {
        try {
            if (Trillium.moduleManager.getModuleByClass(XRay.class).isOn() && Trillium.moduleManager.getModuleByClass(XRay.class).wh.getValue() ) {
